...
namespace NSPC.Common
{
    ...
    {
        public static string CombineUnixPath(params string[] paths)
        {
            return Path.Combine(paths).Replace("\\", "/");
        }

        public static string GetTranslation(string code, string language)
        {
            JObject configuration = ConfigCollection.Instance.GetTranslation();
            var token = configuration.SelectToken(code + "." + language);
            if (token == null)
        ...
        public static async Task<Image> ThumbnailImageAsync(string originalImagePath,
            ...
            int? height)
        {
            var fi = new FileInfo(originalImagePath);
            // check mime type
            ...
            try
            {
                ...
                var imageSize = new Size(width ?? originalImage.Width, height ?? originalImage.Height);
                var imThumbnailImage = ResizeImage(originalImage, imageSize);
        ...
        public static string FetchHost(string url)
        {
            var _host = Utils.GetConfig("StaticFiles:Host");

            if (string.IsNullOrEmpty(url))
                return string.Format("{0}/assets/images/avatar.jpeg", _host);

            if (url.StartsWith(_host) || url.StartsWith("http://") || url.StartsWith("https://"))
                return url;
        ...
        public static async Task<string> DownloadAvatar(string url, string userId)
        {
            try
            {
                string rootPath = Path.Combine(Utils.GetConfig("StaticFiles:Folder"));

                var isExistDirectory = Directory.Exists(Path.Combine(rootPath, "user", "avatar"));
                if (!isExistDirectory) Directory.CreateDirectory(Path.Combine(rootPath, "user", "avatar"));

                var fullfolderPath = Path.Combine(rootPath, "user", "avatar");
                ...
                using (System.Net.WebClient wc = new System.Net.WebClient())
                {
                    ...
                    try
                    {
                        wc.DownloadFile(new Uri(url), Path.Combine(fullfolderPath, filename));
                    }
                ...
                return Path.Combine(fullfolderPath, filename).Replace(rootPath, string.Empty).Replace("\\", "/");
            }
            catch (Exception ex)
            {
                Log.Error(ex, "");
        ...
        public static string ExtractAllTextFromHtml(string htmlContent)
        {
            // Load HTML content into HtmlDocument
            HtmlDocument doc = new HtmlDocument();
            doc.LoadHtml(htmlContent);

            // Get plain text from HtmlDocument
        ...
        {
            if (length < 8)
                throw new ArgumentException("Password length should be at least 8 characters for security reasons.");
        ...
        private static int GetRandomNumber(int max)
        {
            using (var rng = new RNGCryptoServiceProvider())
            {
                ...
                return (int)(BitConverter.ToUInt32(randomNumber, 0) % (uint)max);
            }
        }
        ...
        public static string GetSubDomain(string host, string mainDomain)
        {
        ...
        public static string GenerateAvatar(string name, int size = 200)
        {
            ...
            // Convert RGB to hex
            string backgroundColor = $"{r:X2}{g:X2}{b:X2}";

            // Tính độ sáng theo công thức ITU-R BT.601
            int brightness = (299 * r + 587 * g + 114 * b) / 1000;

            // Nếu độ sáng lớn hơn 128 thì chữ màu đen, ngược lại chữ màu trắng
            string textColor = brightness > 128 ? "000" : "fff";

            // Tách chuỗi thành mảng các từ
            var words = name.Split(' ', StringSplitOptions.RemoveEmptyEntries);

            // Lấy chữ cái đầu tiên của từ cuối cùng
            char firstLetterOfLastWord = words[^1][0];

            var url = $"https://ui-avatars.com/api/?name={firstLetterOfLastWord}&size={size}&background={backgroundColor}&color={textColor}&length=1";
            return url;
        }
        public static string TransformAddress(string address, string commune, string district, string province)
        {
            var result = new List<string> { address, commune, district, province }
