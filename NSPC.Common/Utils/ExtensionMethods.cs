...
namespace NSPC.Common
{
    ...
    public static class ExtensionMethods
    {
        ...
        public static IOrderedQueryable<T> GeneatOrderBy<T>(this IQueryable<T> source, string propertyName)
        {
            return OrderingHelper(source, propertyName, false, false);
        }

        public static IOrderedQueryable<T> GeneatOrderByDescending<T>(this IQueryable<T> source, string propertyName)
        {
            return OrderingHelper(source, propertyName, true, false);
        }

        public static IOrderedQueryable<T> GeneatThenBy<T>(this IQueryable<T> source, string propertyName)
        {
            return OrderingHelper(source, propertyName, false, true);
        }

        public static IOrderedQueryable<T> GeneatThenByDescending<T>(this IQueryable<T> source, string propertyName)
        {
            return OrderingHelper(source, propertyName, true, true);
        }

        #endregion EF order by name

        #region EF get page

        public static IOrderedQueryable<T> ApplySorting<T>(this IQueryable<T> dbset, string listSortStr)
        {
            var dataSet = dbset.AsQueryable();
            var inputSort = listSortStr.Split(',').ToList();
            var listSort = new List<string>();
            if (!string.IsNullOrEmpty(listSortStr))
            {
                foreach (var sort in inputSort)
                {
                    var sorted = char.ToUpper(sort[1]) + sort.Substring(2);
                    if (!sort.Contains("-"))
                    {
            ...
            for (var i = 0; i < listSort.Count; i++)
            {
                ...
                if (!string.IsNullOrEmpty(sortItem))
                {
                    ...
                    // Add + or - at the beginning if the sortItem doesn't contain any of them
                    if (!sortItem.StartsWith("+") && !sortItem.StartsWith("-"))
                        sortItem = "+" + sortItem;
...

                    {
                        ...
                        if (sortType == '+')
                        {
                            if (i == 0)
                            {
                                dbset = dbset.GeneatOrderBy(propName);
                            }
                            else
                            {
                                dbset = dbset.GeneatThenBy(propName);
                            }
                        }
                        else if (sortType == '-')
                        {
                            if (i == 0)
                            {
                                dbset = dbset.GeneatOrderByDescending(propName);
                            }
                            else
                            {
                                dbset = dbset.GeneatThenByDescending(propName);
                            }
                        }
                    }
                }
            ...
            where T : class
        {
            ...
            {
                var orderedDbSet = dbSet.ApplySorting(query.Sort);
                var totals = await orderedDbSet.CountAsync();
                ...
                var items = await orderedDbSet.Skip(excludedRows).Take(query.Size.Value).ToListAsync();
    ...
    (this IEnumerable<TSource> source, Func<TSource, TKey> keySelector)
        ...
        #endregion Encrypt and Decrypt (extension method)
